...
2. **Sparse Case ($|S_r| < k-1$ for some $r$):**  
   ...
   - **Step C (Modular Divisibility Relation):**  
     ...
     *(If $p_m \neq p_i$, equating $2N = p_m + p_k \mu_m = p_k + p_i Q_k$ gives $p_k(\mu_m - 1) = p_i Q_k - p_m \implies p_i Q_k = p_k(\mu_m - 1) + p_m \ge 2 p_k + p_m > 2 p_k$.)*

   - **Step D (Cardinality and Growth Contradiction):**  
     By definition, $Q_k$ is a product of primes chosen strictly from $\{p_1, \dots, p_{k-1}\} \setminus \{i\}$, all of which are strictly smaller than $p_k$. Since $2N - p_k = p_i Q_k$ and $p_i \ge 3$, $Q_k$ is subject to the mandatory ceiling $Q_k = \frac{2N - p_k}{p_i} < \frac{2N}{3}$:
     - **For $k = 4$:** $|S_k \setminus \{i\}| \le 1$, so $Q_k$ contains at most one prime factor $p_r < p_k$. If $p_m = p_i$, $Q_k \ge 2 p_k + 1$ directly contradicts $Q_k = p_r < p_k$; if $p_m \neq p_i$, $p_i Q_k = p_i p_r \ge 2 p_k + p_m > 2 p_k$ contradicts $p_i, p_r < p_k$ in a 4-cycle.
     - **For $k \ge 5$:** $Q_k$ is a product of at least two distinct primes from $\{p_1, \dots, p_{k-1}\} \setminus \{i\}$. The modular constraint $p_i Q_k \ge 2 p_k + p_m$ forces $2N - p_k = p_i Q_k \ge 3(2 p_k + 1) = 6 p_k + 3 \implies p_k \le \frac{2N - 3}{7}$. The product of these distinct primes $Q_k = \prod_{j \in S_k \setminus \{i\}} p_j$ grows superlinearly, exceeding the mandatory ceiling $Q_k < \frac{2N}{3}$, yielding a direct contradiction.
