...
2. **Sparse Case ($|S_r| < k-1$ for some $r$):**  
   ...
   - **Step D (Cardinality and Growth Contradiction):**  
     ...
     - **For $k = 4$:** $|S_k \setminus \{i\}| \le 1$, so $Q_k$ contains at most one prime factor $p_r < p_k$. This implies $Q_k = p_r < p_k$, which directly contradicts $Q_k > p_k$.
     - **For $k \ge 5$:** $Q_k \ge 2 p_k + 1$ forces the product of smaller primes to grow faster than $N$, contradicting $p_k < \frac{2N}{3}$.
