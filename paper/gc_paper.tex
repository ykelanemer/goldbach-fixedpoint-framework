...
\end{enumerate}
Consequently, fixed-point islands with $p_k \le \sqrt{2N}$ are unconditionally impossible. The remaining frontier of Conjecture~\ref{conj:spectral_decoupling} is strictly compressed to the isolated Chen-type configuration where $p_k > \sqrt{2N}$ and row $m$ is a semiprime $2N - p_m = p_k \cdot q$ with $q \in I$.
\end{proposition}
...
$6 \le k \le 97$ & Isolated Chen-Type Semiprime ($2N - p_m = p_k q$) & \textbf{Singular Remaining Frontier} & Conjecture~\ref{conj:spectral_decoupling} \\ \bottomrule
\end{tabular}%
}
...
\item \textbf{Reduction to the Singular Chen-Type Frontier ($6 \le k \le 97$):}  
   By Proposition~\ref{prop:semiprime_elimination}, all islands with $p_k \le \sqrt{2N}$ and all configurations where the parent cofactor $Q_m$ is composite are unconditionally eliminated via the Semiprime Impossibility Theorem and the Bootstrap Cascade. The remaining open problem is mathematically caged within a single isolated Diophantine equation: $2N - p_m = p_k \cdot q$ with $p_k > \sqrt{2N}$.
\end{enumerate}
...
\begin{quote}
\textbf{Conjecture 5.1 (Spectral Decoupling of the Singular Semiprime Island).}  
